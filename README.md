...
この repo のログは message routing と pane capture を分けて残します。`agent-send` で流れた message は `sender`、`targets`、`msg-id`、`reply-to` を含む `.agent-index.jsonl` に追記され、tmux pane 側の画面は `*.ans` と `*.log` に保存されます。pane log には `.meta` が付き、更新時刻や overwrite 履歴も残ります。
