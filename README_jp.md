...
更新履歴: [docs/updates/README.md](docs/updates/README.md) / [beta 1.0.4 日本語](docs/updates/beta-1.0.4.ja.md)
...
| Chat UI | user と agent、agent 同士の会話、返信、添付、ファイル参照、brief / memory、pane 操作、runtime ヒント表示 |
| Logs | `.agent-index.jsonl` の構造化メッセージログ、`.log` / `.ans` の pane capture、static HTML export |
| Backend | Auto mode、Awake、通知音と browser notification、Hub / chat の PWA 導線、Cron による定時実行、Gemini direct bridge、任意の public 公開（Cloudflare は実装済み） |
...
`/gemini` は通常の pane 駆動 agent とは別系統です。direct API bridge を通して同じ chat タイムラインに結果を返しますが、pane 側の memory や file mutation tool、CLI 固有機能を自動では引き継ぎません。
...
- [docs/updates/beta-1.0.4.ja.md](docs/updates/beta-1.0.4.ja.md): `beta 1.0.3` 以降の変更点
